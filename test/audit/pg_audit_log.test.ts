/**
 * Tests `PgAuditLog` (the Postgres version of the class that provides an audit log
 * backed only by the RDB; `src/audit/pg_audit_log.ts`). Verifies the same aspects as
 * `test/audit/sqlite_audit_log.test.ts` using PGlite (an in-process WASM Postgres), the
 * `audits` table in `test/test_support/fixtures/pg_schema.ts`, and `pg_migrations`
 * (the same migration-application approach as `test/jobs/pg_database_job_queue.test.ts`).
 */
import { PGlite } from "@electric-sql/pglite";
import { drizzle } from "drizzle-orm/pglite";
import { migrate } from "drizzle-orm/pglite/migrator";
import { afterEach, beforeEach, describe, expect, test, vi } from "vite-plus/test";
import { PgAuditLog } from "../../src/audit/pg_audit_log.js";
...
/** Deterministic ID-generator stub for tests (same approach as `test/jobs/pg_database_job_queue.test.ts`). */
...
describe("PgAuditLog", () => {
	...
	test("record inserts one row with id/createdAt set automatically", async () => {
		const auditLog = new PgAuditLog(ctx.db, schema.audits, {
