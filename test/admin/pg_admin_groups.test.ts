/**
 * Tests `PgAdminGroups` (the Postgres version of admin-panel operator groups;
 * `src/admin/pg_admin_groups.ts`). Verifies the same aspects as
 * `test/admin/sqlite_admin_groups.test.ts` using PGlite (an in-process WASM
 * Postgres), the `adminGroups`/`adminUserGroups` tables in
 * `test/test_support/fixtures/pg_schema.ts`, and `pg_migrations` (the same
 * harness as `test/admin/pg_admin_accounts.test.ts`). The membership table has
 * no foreign keys, so user ids are plain strings here and no `admin_users`
 * rows are needed.
 */
...
import { PgAdminGroups } from "../../src/admin/pg_admin_groups.js";
