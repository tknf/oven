/**
 * Verifies `PgDatabaseJobWorker` (the Postgres variant of the consumer side that
 * backs the queue with the relational database alone; `src/jobs/
 * pg_database_job_worker.ts`). Checks the same aspects as
 * `test/jobs/sqlite_database_job_worker.test.ts` against PGlite (an in-process
 * WASM Postgres) plus the `jobs` table from `test/test_support/fixtures/
 * pg_schema.ts` and `pg_migrations`.
 */
...
import { PgDatabaseJobWorker } from "../../src/jobs/pg_database_job_worker.js";
...
describe("PgDatabaseJobWorker", () => {
	...
	test("performs a runnable row, and on success deletes the row and returns the count processed", async () => {
		...
		const worker = new PgDatabaseJobWorker(ctx.db, schema.jobs, registry);
	...
	test("when perform fails, attempts increases, and if below maxAttempts, runAt advances by the backoff and lockedAt returns to null", async () => {
		...
		const worker = new PgDatabaseJobWorker(
			ctx.db,
			schema.jobs,
			registry,
	...
	test("upon reaching maxAttempts, failedAt/lastError are set and the row is no longer processed by runOnce", async () => {
		...
		const worker = new PgDatabaseJobWorker(ctx.db, schema.jobs, registry, { maxAttempts: 3 });
		const processed = await worker.runOnce();
	...
	test("an unregistered job name sets failedAt, fires onUnknownJob, and doesn't retry", async () => {
		...
		const worker = new PgDatabaseJobWorker(ctx.db, schema.jobs, registry, {}, { onUnknownJob });
		const processed = await worker.runOnce();
	...
	test("when there are more rows than batchSize, only batchSize rows are processed, in ascending runAt order", async () => {
		...
		const worker = new PgDatabaseJobWorker(ctx.db, schema.jobs, registry, { batchSize: 2 });
		const processed = await worker.runOnce();
