...
- **`@tknf/oven/admin` also ships operator accounts to back `auth`** —
  ...
  ≥ 1, and the table must carry both columns — the constructor throws
  otherwise). `authenticate` then locks an account after `maxAttempts`
  consecutive failures, keeps enumeration safety (a locked account returns
  the same `null` a wrong password gets, verified against the real hash for
  timing parity), resets the counter on success, and unlocks on its own once
  `lockDurationSeconds` elapses or via `unlockUser(userId)` (works off
  column presence alone, independent of the `lockout` option). It's a
  deliberate availability trade-off — an attacker who knows a username can
  lock that account — so pair it with `AdminPanel`'s `rateLimiter` (see
  above) rather than relying on either alone; see `docs/admin-accounts.md`'s
  Gotchas for the full reasoning.
- **Opt-in RFC 6238 TOTP two-factor auth**: spread `sqliteAdminUserTotpColumns()`
  (`totpSecret`/`totpEnabledAt`/`totpLastUsedStep`) alongside
  `sqliteAdminUserColumns()` — no constructor option, unlike lockout; column
  presence alone unlocks `beginTotpEnrollment(userId, { issuer, accountName?
})` (generates+stores a Base32 secret, returns it with a scannable
  `otpauthUrl`; QR rendering is app-side), `confirmTotpEnrollment(userId,
code)` (verifies against the pending secret, only then sets
  `totpEnabledAt`), `verifyTotp(userId, code)`, and `disableTotp(userId)`.
  `verifyTotp`'s replay guard is a single atomic conditional UPDATE (`step`
  must be strictly greater than the stored `totpLastUsedStep`) — a code is
  usable at most once even under concurrent requests. `AdminPanel`'s login
  gets a second step automatically per-operator, by shape — no option to
  set: `accounts.users.verifyTotp` present AND the row's `totpEnabledAt`
  non-null. Password success then redirects to `/login/totp` (5-minute
  pending window) instead of setting the identity; `rateLimiter`, when
  wired, protects this step too (same 5-per-5-minutes budget, key
  `` `admin-totp:${pendingUserId}` ``, distinct from the password step's
  key). `totpSecret` is stored as plain text — encrypt the column yourself
  if a DB compromise exposing it is a concern; see `docs/admin-accounts.md`'s
  "Add TOTP two-factor authentication" for the full flow.
