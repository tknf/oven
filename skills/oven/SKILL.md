...
| `@tknf/oven/admin`      | `AdminPanel` (extends RouteHandler), `AdminResource`, `fieldsFromTable`, `SQLiteAdminAccounts`/`PgAdminAccounts`/`MySqlAdminAccounts` (+ `sqliteAdminUsersTable`/`sqliteAdminUserLockoutColumns` etc.), `SQLiteAdminGroups`/`PgAdminGroups`/`MySqlAdminGroups` (+ `sqliteAdminGroupsTable`/`sqliteAdminUserGroupsTable` etc.), `resourcePermission`/`resourcePermissions` |
