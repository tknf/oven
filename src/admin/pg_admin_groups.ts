/**
 * Postgres (pg-core) implementation of admin-panel operator groups: a groups
 * table (id, unique name, a JSON permission set, timestamps), a membership
 * table (userId/groupId pairs), and a service class (`PgAdminGroups`) for
 ...
 * `sqlite_admin_groups.ts` for pg-core (dialect-specific parallel
 * implementation; see the module JSDoc of `pg_model.ts`). See
 ...
 * Postgres-specific decisions are documented here.
 *
 * Postgres-specific column decisions: `createdAt`/`updatedAt` store epoch ms
 * and use `bigint(..., { mode: "number" })` because a 32-bit `integer` would
 * go out of range (same reason as `createdAt`/`updatedAt` in `pg_model.ts`).
 * pg-core supports `.returning()`, so the write paths are identical to the
 * SQLite version.
 *
 * The type of `db` is `PgDatabase<TQueryResult, TSchema>` (see the module
 * JSDoc of `pg_model.ts` for why `TQueryResult` is promoted to a class type
 * parameter), and `TSchema` is generic for the same reason as
 * `SQLiteAdminGroups`.
 */
...
/**
 * The type of a Drizzle table with the columns required by `PgAdminGroups`
 * for the groups table. Uses `AnyPgColumn` (the same idea as
 * `PgAdminUserRecordTable`) and does not care about the table name or other
 * column layout, so a table extended with app-specific columns still
 * satisfies it.
 */
export type PgAdminGroupRecordTable = PgTable<TableConfig> & {
 ...
 */
export type PgAdminUserGroupRecordTable = PgTable<TableConfig> & {
	userId: AnyPgColumn<{ data: string; notNull: true }>;
	groupId: AnyPgColumn<{ data: string; notNull: true }>;
	createdAt: AnyPgColumn<{ data: number; notNull: true }>;
};

/**
 * Row type of a table satisfying `PgAdminGroupRecordTable`, derived from
 * `$inferSelect` (same technique as `PgAdminUserRecord`).
 */
export type PgAdminGroupRecord<TGroups extends PgAdminGroupRecordTable> = TGroups["$inferSelect"];
...
/** Options for constructing a `PgAdminGroups`. */
export type PgAdminGroupsOptions = {
	/** `IdGenerator` used for id generation. Defaults to `SnowflakeIdGenerator` (same convention as `PgAdminAccounts`). */
	idGenerator?: IdGenerator;
};

/**
 * Operator-group service backed by two Drizzle pg-core tables: a groups table
 * satisfying `PgAdminGroupRecordTable` and a membership table satisfying
 * `PgAdminUserGroupRecordTable`. Both tables are required — feature presence
 * is expressed at the type level by constructing the class, never by optional
 * tables. Generic over the concrete tables so an extended groups table keeps
 * its extra columns typed on returned rows.
 */
export class PgAdminGroups<
	TGroups extends PgAdminGroupRecordTable,
	TUserGroups extends PgAdminUserGroupRecordTable,
...
> {
	...
	/**
	 * Internal helpers for referring to the injected tables as `PgTable` (the
	 * concrete type with the generic type parameter erased). pg-core's
	 * `.from()`/`.insert()`/`.update()`/`.delete()` (and the JOIN builder's
	 * result-type bookkeeping) have overloads keyed on conditional types that
	 * cannot resolve for an abstract type parameter and cause a compile error
	 * (same pg-core-specific issue and workaround as `PgAdminAccounts#pgTable`;
	 * see its JSDoc). Every Drizzle verb call site uses these getters instead
	 * of the fields directly (column references keep using the fields).
	 */
	private get pgGroupsTable(): PgTable {
		return this.groups;
	}

	/** See `pgGroupsTable`. */
	private get pgUserGroupsTable(): PgTable {
		return this.members;
	}
	 ...
	 * name is pre-checked via `findByName` and throws; the table's UNIQUE
	 * index is the last line of defense, so a concurrent insert racing past
	 * the pre-check surfaces as a raw driver error. `permissions` defaults to
	 * `[]`. Returns the created row.
	 */
	async createGroup(input: {
		name: string;
		permissions?: readonly string[];
	}): Promise<PgAdminGroupRecord<TGroups>> {
		const name = normalizeGroupName(input.name);
		if (name === "") {
			throw new Error("PgAdminGroups#createGroup: group name must not be empty");
		}
		const existing = await this.findByName(name);
		if (existing !== undefined) {
			throw new Error(`PgAdminGroups#createGroup: group name "${name}" is already taken`);
		}
		const now = Date.now();
		const values = {
		 ...
		 * `PgAdminAccounts#createUser`).
		 */
		const [row] = await this.db
			.insert(this.pgGroupsTable)
	...
	async updateGroup(
		groupId: string,
		patch: { name?: string },
	): Promise<PgAdminGroupRecord<TGroups> | undefined> {
		const update: Record<string, unknown> = {};
		if (patch.name !== undefined) {
			const name = normalizeGroupName(patch.name);
			if (name === "") {
				throw new Error("PgAdminGroups#updateGroup: group name must not be empty");
			}
			const existing = await this.findByName(name);
			if (existing !== undefined && this.baseRow(existing).id !== groupId) {
				throw new Error(`PgAdminGroups#updateGroup: group name "${name}" is already taken`);
			}
			update.name = name;
		}
	...
	async deleteGroup(groupId: string): Promise<void> {
		await this.db.delete(this.pgUserGroupsTable).where(eq(this.members.groupId, groupId));
		await this.db.delete(this.pgGroupsTable).where(eq(this.groups.id, groupId));
	}

	/** Fetches a single group by id. Returns `undefined` if it doesn't exist. */
	async retrieve(groupId: string): Promise<PgAdminGroupRecord<TGroups> | undefined> {
		const [row] = await this.db
			.select()
			.from(this.pgGroupsTable)
			.where(eq(this.groups.id, groupId))
			.limit(1);
		return row;
	}
	...
	async findByName(name: string): Promise<PgAdminGroupRecord<TGroups> | undefined> {
	...
	async listGroups(): Promise<PgAdminGroupRecord<TGroups>[]> {
		return this.db.select().from(this.pgGroupsTable).orderBy(asc(this.groups.name));
	}
	 ...
	 * (same reasoning as `PgAdminAccounts#setUserPermissions`). A missing
	 * group is a no-op.
	 */
	...
	async setUserGroups(userId: string, groupIds: readonly string[]): Promise<void> {
		...
		await this.db.delete(this.pgUserGroupsTable).where(eq(this.members.userId, userId));
		if (deduped.length === 0) return;
		const now = Date.now();
		const rows = deduped.map((groupId) => ({ userId, groupId, createdAt: now }));
		/** `as` for the same generic-`$inferInsert` reason as `createGroup`. */
		await this.db.insert(this.pgUserGroupsTable).values(rows as TUserGroups["$inferInsert"][]);
	}
	 ...
	 */
	async userGroups(userId: string): Promise<PgAdminGroupRecord<TGroups>[]> {
		/**
		...
		const rows = await this.db
			.select(getTableColumns(this.pgGroupsTable))
			.from(this.pgUserGroupsTable)
			.innerJoin(this.pgGroupsTable, eq(this.members.groupId, this.groups.id))
			.where(eq(this.members.userId, userId))
			.orderBy(asc(this.groups.name));
		return rows as PgAdminGroupRecord<TGroups>[];
	}
	 ...
	 * permission set (`PgAdminAccounts#userPermissions`); the superuser bypass
	 ...
	 * but every table satisfying `PgAdminGroupRecordTable` structurally carries
	 * these columns with exactly these data types, so `as` is used only here
	 * (same justification style as `PgAdminAccounts#baseRow`).
	 */
	private baseRow(row: PgAdminGroupRecord<TGroups>): AdminGroupBaseRow {
		return row as AdminGroupBaseRow;
	}
}

/**
 * Factory that returns a default schema satisfying `PgAdminGroupRecordTable`.
 * The table name can be changed via the `tableName` argument (defaults to
 * `"admin_groups"`). Migration generation is left to the app via drizzle-kit
 * (this factory only provides the schema definition).
 */
export const pgAdminGroupsTable = (tableName = "admin_groups") =>
	pgTable(
		tableName,
		{
...
export const pgAdminUserGroupsTable = (tableName = "admin_user_groups") =>
	...
	) satisfies PgAdminUserGroupRecordTable;
