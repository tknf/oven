 ...
 * module JSDoc): spread `mysqlAdminUserLockoutColumns()` into the table and
 * pass a `lockout` option to the constructor. `lockedUntil` uses
 ...
 * lockout columns (see `mysqlAdminUserLockoutColumns`), following the same
 * per-field `AnyMySqlColumn<{...}>` idiom as `MySqlAdminUserRecordTable`.
 * `MySqlAdminAccountsOptions#lockout` requires the constructor's table to
 * satisfy this (checked at runtime via `hasLockoutColumns`, since the base
 * `MySqlAdminUserRecordTable` contract does not guarantee it).
 */
export type MySqlAdminUserLockoutRecordTable = MySqlAdminUserRecordTable & {
	failedAttempts: AnyMySqlColumn<{ data: number; notNull: true }>;
	lockedUntil: AnyMySqlColumn<{ data: number; notNull: false }>;
};

/**
 * Runtime type guard for `MySqlAdminUserLockoutRecordTable`: whether `table`
 ...
 * construction (when `MySqlAdminAccountsOptions#lockout` is set) and again by
 * `unlockUser` (which works off column presence alone, independent of whether
 * `lockout` was configured).
 */
const hasLockoutColumns = <TUsers extends MySqlAdminUserRecordTable>(
	table: TUsers,
): table is TUsers & MySqlAdminUserLockoutRecordTable =>
 ...
 * `MySqlAdminAccountsOptions#lockout`. Requires the constructor's table to
 * satisfy `MySqlAdminUserLockoutRecordTable` (spread
 * `mysqlAdminUserLockoutColumns()` into it) — the constructor throws
 * otherwise. See `MySqlAdminAccounts#authenticate` for the algorithm.
 */
export type MySqlAdminAccountsLockoutOptions = {
...
export type MySqlAdminAccountsOptions = {
	 ...
	 */
	lockout?: MySqlAdminAccountsLockoutOptions;
};
...
> {
	...
	private readonly lockout: MySqlAdminAccountsLockoutOptions | undefined;
	/**
	 * The constructor's table, narrowed to `MySqlAdminUserLockoutRecordTable`
	...
	constructor(
		...
		if (options.lockout !== undefined) {
			if (options.lockout.maxAttempts < 1) {
				throw new Error("MySqlAdminAccounts: lockout.maxAttempts must be at least 1");
			}
			if (options.lockout.lockDurationSeconds < 1) {
				throw new Error("MySqlAdminAccounts: lockout.lockDurationSeconds must be at least 1");
			}
			if (this.lockoutColumns === undefined) {
				throw new Error(
					"MySqlAdminAccounts: lockout requires failedAttempts/lockedUntil columns on the table — spread mysqlAdminUserLockoutColumns() into it",
				);
			}
		}
	 ...
	 *
	 * **When `MySqlAdminAccountsOptions#lockout` is configured** (and only
	...
	async authenticate(credentials: {
		...
		} as MySqlAdminUserRecord<TUsers>;
	}
	 ...
	 * `MySqlAdminAccountsOptions#lockout` was passed to the constructor, and
	 * throws when the columns are absent.
	 */
	async unlockUser(userId: string): Promise<void> {
		if (this.lockoutColumns === undefined) {
			throw new Error(
				"MySqlAdminAccounts#unlockUser: the table has no failedAttempts/lockedUntil columns — spread mysqlAdminUserLockoutColumns() into it",
			);
		}
		await this.db
			.update(this.table)
	 ...
	 */
	private lockoutBaseRow(row: MySqlAdminUserRecord<TUsers>): AdminUserLockoutBaseRow {
	 ...
	 * threshold check). Unlike `lastActiveSuperuserGuardCondition`, this does
	 * not read the table in a subquery — it only references the row's own
	 * columns in the `SET` clause of the same statement — so MySQL's
	 * `ER_UPDATE_TABLE_USED` restriction does not apply here. Called from
	 * `authenticate` for a wrong password on an unlocked account.
	 */
	private async recordFailedAttempt(
		lockoutTable: TUsers & MySqlAdminUserLockoutRecordTable,
		lockout: MySqlAdminAccountsLockoutOptions,
		userId: string,
	): Promise<void> {
 ...
 * columns. Spread alongside `mysqlAdminUserColumns()` into the same table to
 * enable `MySqlAdminAccountsOptions#lockout`:
 *
 * ```ts
 * export const adminUsers = mysqlTable(
 *   "admin_users",
 *   { ...mysqlAdminUserColumns(), ...mysqlAdminUserLockoutColumns() },
 ...
 * — only `MySqlAdminAccounts#authenticate` and `#unlockUser` ever write them.
 * See `authenticate`'s JSDoc for the algorithm.
 */
